...
import { Layers, Sparkles, Users2, Orbit, Brain, Rocket } from "lucide-react";
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
          {features.map((feat, i) => {
            ...
            return (
              ...
              >
                {/* Hover glow */}
                <div className={`absolute inset-0 rounded-3xl bg-gradient-to-b from-${feat.glowColor}-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none`} />
                
                <div
                  className={`relative w-12 sm:w-14 h-12 sm:h-14 rounded-2xl flex items-center justify-center border ${feat.iconBg} ${feat.iconColor} ${feat.iconBorder} transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-${feat.glowColor}-500/20`}
                >
                ...
                <div className="relative mt-4 sm:mt-6">
                  <span className="text-xs font-medium text-slate-500 group-hover:text-violet-400 transition-colors cursor-pointer">
                    Learn more →
                  </span>
                </div>
              </motion.div>
